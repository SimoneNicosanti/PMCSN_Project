...
public class EventProcessor<T> {
    ...
    public List<Event<RiderGroup>> processEvent(Event<T> event) {
        ...
        switch (event.getEventType()) {
            case ARRIVAL :
                center.arrival(job, ClockHandler.getInstance().getClock());
                nextEvents = generateNextEventsFromArrival(event) ;
                break;

            case START_PROCESS :
                double serviceTime = center.startService(ClockHandler.getInstance().getClock()) ;
    ...
    private List<Event<RiderGroup>> generateNextEventsFromArrival(Event<T> event) {
        ...
        if (event.getEventCenter() instanceof Attraction) {
            ...
            if (!attraction.isServing()) {
                Event<RiderGroup> newEvent = new Event<RiderGroup>(attraction, EventType.START_PROCESS, ClockHandler.getInstance().getClock(), (RiderGroup) event.getJob()) ;
    ...
    private List<Event<RiderGroup>> generateNextEventsFromStart(Event<T> event, double serviceTime) {
        ...
        if (event.getEventCenter() instanceof Attraction) {
            ...
            Event<RiderGroup> newEvent = new Event<RiderGroup>(attraction, EventType.END_PROCESS, ClockHandler.getInstance().getClock() + serviceTime, null) ;
            newEventList.add(newEvent) ;
        }
        //if (event.getEventCenter() instanceof Entrance) {}
        //if (event.getEventCenter() instanceof Restaurant) {}
    ...
    private List<Event<RiderGroup>> generateNextEventsFromEnd(Event<T> event, List<T> completedJobList) {
        ...
        if (event.getEventCenter() instanceof Attraction) {
            for (T completedJob : completedJobList) {
                ...
                if (areaProbability <= RESTAURANT_AREA_THR) {
                    ...
                    */ 
                    Attraction nextAttraction = attractionRoutingComputer.computeNextAttractionForJob((RiderGroup) completedJob) ;
                    Event<RiderGroup> newEvent = new Event<RiderGroup>(nextAttraction, EventType.ARRIVAL, ClockHandler.getInstance().getClock(), (RiderGroup) completedJob) ;
                    newEventList.add(newEvent) ;
         ...
         *      if end --> schedule start with same current time (start and end are the same) and schedule arrival to other
